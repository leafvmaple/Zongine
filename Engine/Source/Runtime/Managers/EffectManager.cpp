...
namespace Zongine {
    static struct _SHADER_TEMPLATE {
        ...
        },
        {
            MATERIAL_SHADER_ROOT"FlexMeshShader.fx5",
            {
                ...
                {"FLEX_POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 1,  D3D11_APPEND_ALIGNED_ELEMENT,   D3D11_INPUT_PER_VERTEX_DATA, 0},
                {"MIX_FACTOR",    0, DXGI_FORMAT_R32_FLOAT,          1,  D3D11_APPEND_ALIGNED_ELEMENT,   D3D11_INPUT_PER_VERTEX_DATA, 0},
            }
        },
    };
    ...
    bool EffectManager::Initialize() {
        ...
        device->CreatePixelShader(blob->GetBufferPointer(), blob->GetBufferSize(), nullptr, m_OITBlendPS.GetAddressOf());
